...
// Minimal theme til landing (matcher App classic-tema)
const T = {
...
const FEATURES = [
  { icon: "💬", title: "AI-drevet chat",      desc: "Stil spørgsmål om alle fag og få forklaringer der passer præcis til dit niveau – ikke færdige svar." },
  { icon: "📝", title: "Smarte noter",        desc: "Skriv og organiser dine noter med en rig teksteditor. Fed, punktlister, overskrifter – ligesom Google Docs." },
  { icon: "📚", title: "Alle fag samlet",     desc: "Matematik, dansk, engelsk, programmering, naturfag, historie og mere – alt ét sted." },
  { icon: "🛡️", title: "Lær, ikke snyd",      desc: "FagAI skriver aldrig dine opgaver. Den guider dig til at forstå og tænke selv." },
  { icon: "🎨", title: "Dit eget tema",       desc: "Vælg mellem fem smukke farvetemaer og gør appen til din egen." },
  { icon: "⚡", title: "Lynhurtig hjælp",     desc: "Få svar på sekunder. Ingen ventetid, ingen distraktioner – bare læring." },
];
    ...
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: "#faf6ee", color: "#2c1810", overflowX: "hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .nav-auth-btn{transition:all 0.16s ease;cursor:pointer;border:none;}
        .nav-auth-btn:hover{transform:translateY(-1px);filter:brightness(1.06);}
        ::-webkit-scrollbar{width:6px;}
        ::-webkit-scrollbar-thumb{background:#d4b896;border-radius:3px;}
        .brand-link{transition:opacity 0.15s;cursor:pointer;}
      ...
      <nav style={{
        ...
        borderBottom: scrolled ? "1px solid #e0d0ba" : "none",
        display: "flex", alignItems: "center", padding: "0 48px",
        transition: "all 0.3s ease",
        boxShadow: scrolled ? "0 2px 16px rgba(92,30,15,0.08)" : "none",
      }}>
        {/* Brand — scroller til toppen af landingpagen */}
        <div className="brand-link" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 28 }}>
          ...
          <div style={{ width: 1, height: 20, background: "#d4b896" }}/>

          {user ? (
            /* Logget ind → kun "Gå til chat" */
            <button className="nav-auth-btn" onClick={() => navigate("/chat")}
              style={{ padding: "9px 24px", background: "linear-gradient(135deg,#5c1e0f,#9e3a22)", color: "#f1e194", borderRadius: 10, fontSize: 14, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 3px 12px rgba(92,30,15,0.3)" }}>
              Gå til chat →
            </button>
          ) : (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <button className="nav-auth-btn"
                ...
                style={{ padding: "9px 20px", background: "transparent", color: "#5c1e0f", border: "1.5px solid #c9a96e", borderRadius: 10, fontSize: 14, fontWeight: 600, fontFamily: "'DM Sans',sans-serif" }}>
                Log ind
              </button>
              <button className="nav-auth-btn"
                onClick={() => openAuth("register")}
                style={{ padding: "9px 22px", background: "linear-gradient(135deg,#5c1e0f,#9e3a22)", color: "#f1e194", borderRadius: 10, fontSize: 14, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 3px 12px rgba(92,30,15,0.3)" }}>
                Registrér
              </button>
            </div>
          )}
      ...
      <section style={{
        ...
        <h1 style={{ fontSize: "clamp(42px,6vw,80px)", fontWeight: 800, lineHeight: 1.1, fontFamily: "'Lora',serif", letterSpacing: "-1.5px", marginBottom: 24, animation: "fadeUp 0.7s ease 0.1s both", maxWidth: 820 }}>
          Lær smartere.<br/>
        ...
        <p style={{ fontSize: "clamp(16px,2vw,20px)", color: "#6b4438", lineHeight: 1.7, maxWidth: 580, marginBottom: 44, animation: "fadeUp 0.7s ease 0.2s both" }}>
        ...
        <div style={{ animation: "fadeUp 0.8s ease 0.4s both, float 5s ease-in-out 1s infinite", maxWidth: 860, width: "100%", position: "relative" }}>
          ...
          <div style={{ background: "white", borderRadius: 20, overflow: "hidden", border: "2px solid #e0d0ba", boxShadow: "0 32px 80px rgba(92,30,15,0.18), 0 8px 24px rgba(92,30,15,0.1)", position: "relative", zIndex: 1 }}>
            {/* Mock topbar */}
            <div style={{ height: 48, background: "#5c1e0f", display: "flex", alignItems: "center", padding: "0 20px", gap: 16, borderBottom: "2px solid #b8a93a" }}>
              ...
              <div style={{ display: "flex", gap: 3, background: "rgba(0,0,0,0.2)", borderRadius: 9, padding: 3, marginLeft: 8 }}>
                {["Chat","Noter","Fag"].map(t => (
                  <div key={t} style={{ padding: "4px 14px", borderRadius: 7, background: t==="Chat"?"#f1e194":"transparent", color: t==="Chat"?"#5c1e0f":"rgba(241,225,148,0.7)", fontSize: 12, fontWeight: 600 }}>{t}</div>
                ))}
              </div>
            </div>
            {/* Mock content */}
              ...
              <div style={{ flex: 1, padding: "20px 24px", display: "flex", flexDirection: "column", gap: 14, background: "#faf6ee" }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  ...
                  <div style={{ background: "white", borderRadius: "4px 14px 14px 14px", padding: "10px 14px", fontSize: 12.5, color: "#2c1810", border: "1.5px solid #e0d0ba", lineHeight: 1.6, maxWidth: 360, fontFamily: "'Lora',serif" }}>
                    ...
                    <strong>En andengradsligning</strong> er en ligning, hvor x står med højeste potens 2...
                  </div>
                </div>
              </div>
            </div>
      ...
      <section id="funktioner" style={{ padding: "100px 48px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div ref={setRef("features-title")} data-section="features-title" className={`reveal${visible("features-title") ? " show" : ""}`} style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
            {FEATURES.map((f, i) => (
              <div key={i} ref={setRef("feat-"+i)} data-section={"feat-"+i} className={`feature-card reveal${visible("feat-"+i) ? " show" : ""}`}
                style={{ background: "white", borderRadius: 20, padding: "30px 28px", border: "2px solid #e0d0ba", boxShadow: "0 4px 16px rgba(92,30,15,0.07)", transitionDelay: `${i * 0.07}s` }}>
      ...
      <section style={{ padding: "80px 48px 100px", background: "linear-gradient(160deg,#5c1e0f 0%,#7a2a17 100%)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <div ref={setRef("how")} data-section="how" className={`reveal${visible("how") ? " show" : ""}`} style={{ textAlign: "center", marginBottom: 60 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
            {[
              { step: "01", title: "Opret konto", desc: "Registrér dig med din e-mail og vælg en adgangskode.", icon: "🚀" },
              ...
              <div key={i} ref={setRef("step-"+i)} data-section={"step-"+i} className={`reveal${visible("step-"+i) ? " show" : ""}`} style={{ textAlign: "center", transitionDelay: `${i*0.1}s` }}>
      ...
      <section id="om" style={{ padding: "100px 48px" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <div ref={setRef("testimonials")} data-section="testimonials" className={`reveal${visible("testimonials") ? " show" : ""}`} style={{ textAlign: "center", marginBottom: 60 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
            {TESTIMONIALS.map((t, i) => (
              <div key={i} ref={setRef("test-"+i)} data-section={"test-"+i} className={`reveal${visible("test-"+i) ? " show" : ""}`}
      ...
      <section style={{ padding: "100px 48px", background: "linear-gradient(135deg,#faf6ee,#f2ead8)", borderTop: "1px solid #e0d0ba" }}>
        <div ref={setRef("cta")} data-section="cta" className={`reveal${visible("cta") ? " show" : ""}`} style={{ maxWidth: 680, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 17, color: "#6b4438", lineHeight: 1.7, marginBottom: 40 }}>
            Opret din konto og begynd at lære med FagAI.
          </p>
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            ...
            <button className="cta-btn" onClick={() => openAuth("login")}
              style={{ padding: "18px 36px", background: "white", color: "#5c1e0f", borderRadius: 16, fontSize: 18, fontWeight: 600, border: "2px solid #e0d0ba", boxShadow: "0 4px 16px rgba(92,30,15,0.08)" }}>
              Log ind
            </button>
          </div>
        </div>
      ...
      <footer style={{ background: "#5c1e0f", padding: "32px 48px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <p style={{ fontSize: 12, color: "rgba(241,225,148,0.5)", fontFamily: "'DM Sans',sans-serif" }}>
          © 2026 FagAI · Lavet til danske elever
        </p>
        <button className="cta-btn" onClick={() => openAuth("register")}
